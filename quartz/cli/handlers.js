...
export async function handleBuild(argv) {
  ...
  const build = async (clientRefresh) => {
    ...
    // Post-build: Copy OneSignal & Robots to root
    try {
      const staticDir = path.join(cwd, "quartz", "static")
      const workerSrc = path.join(staticDir, "OneSignalSDKWorker.js")
      const robotsSrc = path.join(staticDir, "robots.txt")
      const workerDest = path.join(argv.output, "OneSignalSDKWorker.js")
      const robotsDest = path.join(argv.output, "robots.txt")

      if (fs.existsSync(workerSrc)) {
        await promises.copyFile(workerSrc, workerDest)
        console.log(styleText("cyan", `  Copied OneSignalSDKWorker.js to ${argv.output}`))
      }
      if (fs.existsSync(robotsSrc)) {
        await promises.copyFile(robotsSrc, robotsDest)
        console.log(styleText("cyan", `  Copied robots.txt to ${argv.output}`))
      }
    } catch (e) {
      console.log(styleText("yellow", `  Warning: Failed to copy static files to root: ${e.message}`))
    }
